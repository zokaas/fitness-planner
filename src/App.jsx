...
function WeightChart({ entries }) {
  if (entries.length < 2) return (
    <div style={{ textAlign:"center",padding:"20px 0",color:"#BBB",fontFamily:"sans-serif",fontSize:12 }}>
  ...
  const points = entries.map((e,i) => `${xScale(i)},${yScale(e.weight_kg)}`).join(" ")
  const areaPoints = `${xScale(0)},${H-PAD.b} ${points} ${xScale(entries.length-1)},${H-PAD.b}`
  const start = vals[0], current = vals[vals.length-1], diff = (current - start).toFixed(1)

  return (
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",marginBottom:8,fontFamily:"sans-serif" }}>
        <div style={{ fontSize:11,color:"#999" }}>Start: <strong style={{ color:"#333" }}>{start} kg</strong></div>
        <div style={{ fontSize:13,fontWeight:700,color:diff<0?"#3D7A52":"#C0392B" }}>{diff>0?"+":""}{diff} kg</div>
        <div style={{ fontSize:11,color:"#999" }}>Now: <strong style={{ color:"#333" }}>{current} kg</strong></div>
      </div>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow:"visible" }}>
        ...
        {[0,.25,.5,.75,1].map(t => {
          ...
          return (
            <g key={t}>
              ...
              <text x={PAD.l-4} y={y+4} textAnchor="end" fontSize={8} fill="#BBB" fontFamily="sans-serif">{v}</text>
            </g>
          )
        })}
        {/* Area fill */}
        ...
        {/* Line */}
        <polyline points={points} fill="none" stroke="#3D7A52" strokeWidth={2} strokeLinejoin="round" />
        {/* Dots */}
        {entries.map((e,i) => (
          <circle key={i} cx={xScale(i)} cy={yScale(e.weight_kg)} r={3} fill="#3D7A52" />
        ))}
        {/* X labels (every Nth) */}
        {entries.map((e,i) => {
          if (entries.length > 10 && i % Math.floor(entries.length/5) !== 0 && i !== entries.length-1) return null
          return <text key={i} x={xScale(i)} y={H-PAD.b+16} textAnchor="middle" fontSize={7} fill="#BBB" fontFamily="sans-serif">{e.date.slice(5)}</text>
        })}
      </svg>
    </div>
  )
              ...
              {dayEx && (
                <div style={{ background:exMeta.bg,border:`1.5px solid ${workoutDone?"#3D7A52":"#E8E8E8"}`,borderRadius:12,padding:14,position:"relative" }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10 }}>
                    <div style={{ flex:1 }} onClick={()=>setSwap({ open:true, mode:"exercise", type:"cardio" })} style={{ cursor:"pointer",flex:1 }}>
                      ...
                      <div style={{ fontSize:9,color:"#CCC",fontFamily:"sans-serif",marginTop:6 }}>TAP TO SWAP ↕</div>
                    </div>
