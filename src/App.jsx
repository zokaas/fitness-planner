...
function SwapDrawer({ open, mode, type, currentId, onSelect, onClose }) {
  ...
  const lib = isMeal
    ? MEAL_LIBRARY[type]
    : EX_TYPES.flatMap((t) =>
        EXERCISE_LIBRARY[t].map((e) => ({ ...e, type: t })),
      );
            ...
            >
              <div
                style={{
                  ...
                  height: 18,
                  borderRadius: 4,
                  border: `2px solid ${homeOnly ? "#C0392B" : "#DDD"}`,
                  background: homeOnly ? "#C0392B" : "#FFF",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  flexShrink: 0,
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    ...
                    {isCur && (
                      <span
                        style={{
                          fontSize: 8,
                          background: isMeal ? meta.accent : em.accent,
                          color: "#fff",
                          borderRadius: 4,
                          padding: "1px 5px",
                          fontFamily: "sans-serif",
                  ...
                  >
                    {isMeal
                      ? `⏱ ${timeLbl(item.time)}`
                      : `⏱ ${durLbl(item.duration)}${item.duration > 0 ? ` · ~${item.kcalBurn} kcal` : ""}`}
                    {!isMeal && item.homeOk && (
                      <span style={{ marginLeft: 6, color: "#4A8C5C" }}>
                        🏠
                      </span>
                    )}
                  </div>
...
function ProgressView({
  ...
  const handleAdd = () => {
    ...
    if (isNaN(w) || w < 20 || w > 300) {
      setErr("Enter a valid weight (20–300 kg)");
      return;
    }
    setErr("");
    onAddWeight(date, w);
  ...
  return (
    ...
    >
      ...
      <div style={{ background: "#FFF", borderRadius: 12, padding: 16 }}>
        ...
        {err && (
          <div
            style={{ fontSize: 11, color: "#C0392B", fontFamily: "sans-serif" }}
          >
            {err}
          </div>
        )}
            ...
            {[...weightLog]
              ...
              .map((e) => (
                ...
                >
                  ...
                  <button
                    onClick={() => onDeleteWeight(e.id || e.date)}
                    style={{
                      background: "none",
                      border: "none",
                      cursor: "pointer",
    ...
    >
      ...
      >
        {cats.map((t) => {
          ...
          const count = isMeal
            ? MEAL_LIBRARY[t].length
            : EXERCISE_LIBRARY[t].length;
          return (
            <button
              key={t}
              onClick={() => {
          ...
          >
            <div
              style={{
                ...
                height: 18,
                borderRadius: 4,
                border: `2px solid ${homeOnly ? meta.accent : "#DDD"}`,
                background: homeOnly ? meta.accent : "#FFF",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
            ...
            <span
              style={{ fontSize: 11, fontFamily: "sans-serif", color: "#666" }}
            >
              Home-friendly only 🏠
            </span>
          </div>
                ...
                {isMeal ? (
                  ...
                  <>
                    ...
                    {item.intensity !== "none" && (
                      <span
                        style={{
                          color: INTENSITY_COLOR[item.intensity],
...
export default function App() {
  ...
  const registerSW = async () => {
    ...
    try {
      await navigator.serviceWorker.register("/fitness-planner/sw.js");
    } catch (e) {
  ...
  const loadPlan = async (userId) => {
    ...
    data.forEach((row) => {
      if (row.meal_type) {
        if (mp[row.day_index]) mp[row.day_index][row.meal_type] = row.item_id;
      } else {
        ep[row.day_index] = row.item_id;
      }
    });
    setMealPlan(mp);
    setExPlan(ep);
  ...
  const toggleComplete = async (type) => {
    ...
    if (exists) {
      setCompletions((prev) =>
        prev.filter((c) => !(c.date === date && c.type === type)),
      );
      await supabase
        .from("completions")
        .delete()
      ...
      await supabase
        .from("completions")
        .upsert(newEntry, { onConflict: "user_id,date,type" });
    }
  };

  const isComplete = (type) => {
    const d = new Date("2025-03-03");
    d.setDate(d.getDate() + dayIndex);
  ...
  const addWeight = async (date, weight_kg) => {
    ...
    setWeightLog((prev) => {
      ...
      return [...filtered, data?.[0] || row].sort((a, b) =>
  ...
  const mealKcal = MEAL_TYPES.reduce((s, t) => {
    const m = getMeal(dayMeals[t]);
    return s + (m?.kcal || 0);
  }, 0);
  ...
  if (loading)
    return (
      ...
      >
        ...
        <div
          style={{ fontFamily: "sans-serif", color: "#9DC88D", fontSize: 13 }}
        >
          Loading your plan…
        </div>
      </div>
  ...
  return (
    ...
    >
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              fontSize: 20,
              opacity: notifStatus === "denied" ? 0.3 : 1,
            }}
          >
            {notifStatus === "granted" ? "🔔" : "🔕"}
          </button>
      ...
      {view === "plan" && (
        <>
          <div
            style={{ display: "flex", background: "#162810", flexShrink: 0 }}
          >
            {WEEK_THEMES.map((th, i) => (
              <button
                ...
                style={{
                  ...
                  padding: "8px 2px",
                  border: "none",
                  background: activeWeek === i ? "#3A6228" : "transparent",
                  color: activeWeek === i ? "#FFF" : "#6A9A5A",
                  fontFamily: "sans-serif",
                  fontSize: 10,
            ...
            {DAY_LABELS.map((d, i) => (
              <button
                ...
                style={{
                  ...
                  padding: "10px 3px",
                  border: "none",
                  background: activeDay === i ? "#1E3214" : "transparent",
                  color: activeDay === i ? "#FFF" : "#999",
                  fontFamily: "sans-serif",
                  fontSize: 11,
          ...
          >
            {[
              ...
              {
                label: "Burn",
                val: dayEx?.kcalBurn > 0 ? `−${dayEx.kcalBurn}` : "Rest",
                color: "#C0392B",
              },
              {
                label: "Net",
                val: `${Math.max(0, netKcal)} kcal`,
                color: netKcal <= 1300 ? "#3D7A52" : "#555",
              },
            ].map((s) => (
              <div
                key={s.label}
...
>>>>>>> 75ea293 (Add shopping list, meal prep, weight goal line)
                      ...
                      <div
                        style={{
                          fontSize: 16,
                          fontWeight: "bold",
                          color: workoutDone ? "#3D7A52" : "#1A1A1A",
                          textDecoration: workoutDone ? "line-through" : "none",
                        }}
                      >
                        {dayEx.name}
                      </div>
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          background: workoutDone ? "#3D7A52" : "#FFF",
                          border: `2px solid ${workoutDone ? "#3D7A52" : "#DDD"}`,
                          borderRadius: 8,
                          padding: "6px 10px",
                          cursor: "pointer",
                          fontFamily: "sans-serif",
                ...
                {MEAL_TYPES.map((type) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: "#FFF",
                        border: `1.5px solid ${done ? "#3D7A52" : "#EBEBEB"}`,
                        borderRadius: 10,
                        padding: "11px 13px",
                      }}
                    >
                      ...
                      >
                        <div
                          style={{ flex: 1, cursor: "pointer" }}
                          onClick={() =>
                            setSwap({ open: true, mode: "meal", type })
                          }
                        >
                          <div
                            style={{
                        ...
                        >
                          ...
                          <button
                            onClick={() => toggleComplete(`meal_${type}`)}
                            style={{
                              background: done ? "#3D7A52" : "#FFF",
                              border: `2px solid ${done ? "#3D7A52" : "#DDD"}`,
                              borderRadius: 6,
                              padding: "4px 8px",
                              cursor: "pointer",
                              fontFamily: "sans-serif",
